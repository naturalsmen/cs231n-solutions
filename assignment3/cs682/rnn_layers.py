...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    i, f, o, g = h[:, :H], h[:, H:2*H], h[:, 2*H:3*H], h[:, 3*H:4*H] # NxH, not sure
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dh[:, :H] *= dnc_prod * g
    dh[:, H:2*H] *= dnc_prod * prev_c
    dh[:, 2*H:3*H] *= dnext_h * next_c_tanh
    dh[:, 3*H:4*H] *= dnc_prod * i
...
def lstm_backward(dh, cache):
    ...
    # for each timestamp, lstm step backward
