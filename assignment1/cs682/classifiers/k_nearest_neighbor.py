...
class KNearestNeighbor(object):
  ...
  def compute_distances_no_loops(self, X):
    ...
    #########################################################################
    # l2 = sqrt(col_sum(Ite - Itr) ** 2)
    #    = sqrt(col_sum(Ite**2 + Itr**2 - 2*Ite*Itr))
    #    = sqrt(col_sum(Ite**2) + col_sum(Itr**2) - 2*Ite.dot(Itr.T))
    test_sum = np.sum(X ** 2, axis = 1) 
    train_sum = np.sum(self.X_train ** 2, axis = 1)
    dot_product = X.dot(self.X_train.T)
    dists = np.sqrt(test_sum.reshape(-1, 1) + train_sum - 2 * dot_product)
    #########################################################################
    #                         END OF YOUR CODE                              #
    #########################################################################
    return dists
